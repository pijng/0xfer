...
func (r *DBRepository) Create(ctx context.Context, f *File) error {
	query := `
	INSERT INTO files (id, secret, filename, content_type, size, created_at, expires_at)
	VALUES (?, ?, ?, ?, ?, ?, ?)
	`
	_, err := r.db.ExecContext(ctx, query, f.ID, f.Secret, f.Filename, f.ContentType, f.Size, f.CreatedAt.Unix(), f.ExpiresAt.Unix())
	if err != nil {
...
func (r *DBRepository) Get(ctx context.Context, id string) (*File, error) {
	query := `
	SELECT id, secret, filename, content_type, size, created_at, expires_at, download_count
	FROM files WHERE id = ?
	`
	...
	err := row.Scan(&f.ID, &f.Secret, &f.Filename, &f.ContentType, &f.Size, &createdAt, &expiresAt, &f.DownloadCount)
...
func (r *DBRepository) GetExpired(ctx context.Context) ([]File, error) {
	...
	for rows.Next() {
		var f File
		var createdAt, expiresAt int64
		if err := rows.Scan(&f.ID, &f.Secret, &f.Filename, &f.ContentType, &f.Size, &createdAt, &expiresAt, &f.DownloadCount); err != nil {
			return nil, fmt.Errorf("scan row: %w", err)
		}
		f.CreatedAt = time.Unix(createdAt, 0)
		f.ExpiresAt = time.Unix(expiresAt, 0)
